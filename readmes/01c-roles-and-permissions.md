...
Этот документ описывает роли в протоколе dHEDGE, их технические полномочия и ограничения. Здесь зафиксировано кто может менять whitelist активов, комиссии, паузы, параметры вывода и кто может вызывать операции с активами пула. Это исходник для оценки централизации и для risk disclosure.

#### poolFactoryOwner

* Права по управлению активами пула
  `PoolManagerLogic.changeAssets()` — poolFactoryOwner может напрямую добавлять и удалять поддерживаемые активы пула через вызов, обходя poolManager.【F:contracts/PoolManagerLogic.sol†L148-L179】
...
* Права по настройкам протокола
  ...
  `PoolFactory.setMaximumFee()` и `setMaximumPerformanceFeeNumeratorChange()` / `setPerformanceFeeNumeratorChangeDelay()` — poolFactoryOwner выставляет глобальные лимиты комиссий и задержку повышения комиссий poolManager.【F:contracts/PoolFactory.sol†L333-L414】
...
* Доступ к деньгам пользователей
  ...
  3. poolFactoryOwner может направлять комиссионную эмиссию на любой адрес через `setDAOAddress()` и вызов `PoolLogic.mintManagerFee()`, который минтит доли DAO и poolManager.【F:contracts/PoolFactory.sol†L276-L322】【F:contracts/PoolLogic.sol†L780-L820】
...
* Права по управлению активами пула
  `Governance.setContractGuard()` — governanceOwner выбирает guard для внешних протоколов, определяя, куда poolManager может отправлять средства пула.【F:contracts/Governance.sol†L38-L55】
...
* Доступ к деньгам пользователей
  ...
  2. governanceOwner может назначить guard контракты, которые затем получают доступ к активам через poolManager.【F:contracts/Governance.sol†L38-L74】
...
* Права по управлению активами пула
  `EasySwapperV2.setAuthorizedWithdrawers()` — easySwapperOwner решает, какие authorizedWithdrawer адреса завершают выводы через EasySwapper, влияя на выпуск активов из пулов.【F:contracts/swappers/easySwapperV2/EasySwapperV2.sol†L479-L485】
...
* Доступ к деньгам пользователей
  ...
  2. easySwapperOwner может назначить authorizedWithdrawer адреса, которые завершают выводы и маршрутизируют активы пользователей.【F:contracts/swappers/easySwapperV2/EasySwapperV2.sol†L479-L485】【F:contracts/swappers/easySwapperV2/EasySwapperV2.sol†L334-L345】
...
* Права по управлению активами пула
  `PoolLimitOrderManager.addAuthorizedKeeper()` / `removeAuthorizedKeeper()` — limitOrderManagerOwner выбирает authorizedKeeper адреса, которые могут исполнять лимитные ордера и перемещать активы через EasySwapper.【F:contracts/limitOrders/PoolLimitOrderManager.sol†L504-L518】
...
* Доступ к деньгам пользователей
  ...
  2. limitOrderManagerOwner может назначать authorizedKeeper адреса, которые затем управляют исполнением ордеров пользователей.【F:contracts/limitOrders/PoolLimitOrderManager.sol†L504-L541】【F:contracts/limitOrders/PoolLimitOrderManager.sol†L212-L265】
  3. Минтинг наград не предусмотрен.
...
* Права по управлению активами пула
  ...
  `PoolTokenSwapper.setManager()` — poolTokenSwapperOwner назначает poolManager или специализированного оператора, который сможет проводить произвольные транзакции через `execTransaction`.【F:contracts/swappers/poolTokenSwapper/PoolTokenSwapper.sol†L290-L295】
...
* Доступ к деньгам пользователей
  ...
  2. poolTokenSwapperOwner может назначать poolManager и swapWhitelist адреса, которые затем будут перемещать активы пулов через своппер.【F:contracts/swappers/poolTokenSwapper/PoolTokenSwapper.sol†L228-L272】【F:contracts/swappers/poolTokenSwapper/PoolTokenSwapper.sol†L276-L302】
  3. Минтинг вознаграждений не предусмотрен.

* Жёсткие ограничения
  При выполнении `execTransaction` poolManager, назначенный poolTokenSwapperOwner, проходит guard-проверки на whitelist активов и пулов, что ограничивает произвольный вывод средств.【F:contracts/swappers/poolTokenSwapper/PoolTokenSwapper.sol†L230-L272】

#### poolManager

* Права по управлению активами пула
  `PoolLogic.execTransaction()` / `execTransactions()` — poolManager выполняет сделки от имени пула через guard-ы.【F:contracts/PoolLogic.sol†L620-L673】
  `PoolManagerLogic.changeAssets()` — poolManager добавляет и удаляет активы пула.【F:contracts/PoolManagerLogic.sol†L148-L179】
  `PoolManagerLogic.setTrader()` / `removeTrader()` / `addMember()` / `removeMember()` — poolManager управляет доступом к ролям и whitelist участников.【F:contracts/Managed.sol†L73-L126】

* Права по настройкам протокола
  `PoolManagerLogic.setFeeNumerator()` — poolManager может снижать комиссии пула.【F:contracts/PoolManagerLogic.sol†L373-L393】
  `PoolManagerLogic.announceFeeIncrease()` → `commitFeeIncrease()` — poolManager повышает комиссии после задержки и предварительного минтинга комиссий.【F:contracts/PoolManagerLogic.sol†L432-L514】
  `PoolManagerLogic.setTraderAssetChangeDisabled()` и `setMinDepositUSD()` — poolManager управляет дополнительными настройками пула.【F:contracts/PoolManagerLogic.sol†L516-L578】
  `PoolLogic.setPoolPrivate()` — poolManager делает пул приватным или публичным.【F:contracts/PoolLogic.sol†L211-L220】

* Доступ к деньгам пользователей
  1. poolManager может выводить средства через сделки `execTransaction()` при условии прохождения guard-ов.【F:contracts/PoolLogic.sol†L620-L654】
  2. poolManager назначает trader, членов пула и может сменить PoolManagerLogic через фабрику, влияя на доступ других ролей.【F:contracts/Managed.sol†L73-L126】【F:contracts/PoolLogic.sol†L879-L887】
  3. poolManager получает вознаграждение через `PoolLogic.mintManagerFee()` и комиссии на вход/выход, которые может начислить себе.【F:contracts/PoolLogic.sol†L780-L820】【F:contracts/PoolLogic.sol†L303-L314】【F:contracts/PoolLogic.sol†L434-L448】

* Жёсткие ограничения
  Все действия poolManager проходят через contract/asset guard, который проверяет whitelist и публичность транзакций; смена активов требует валидный актив и соблюдение максимума активов, а повышение комиссий требует announce → delay → commit.【F:contracts/PoolLogic.sol†L620-L654】【F:contracts/PoolManagerLogic.sol†L168-L214】【F:contracts/PoolManagerLogic.sol†L432-L507】

#### trader

* Права по управлению активами пула
  `PoolLogic.execTransaction()` — trader может выполнять guarded транзакции, если guard пометил операцию как закрытую и разрешает trader адресу действовать.【F:contracts/PoolLogic.sol†L620-L641】
  `PoolManagerLogic.changeAssets()` — trader может менять состав активов, пока poolManager не отключил такую возможность флагом `traderAssetChangeDisabled`.【F:contracts/PoolManagerLogic.sol†L151-L179】【F:contracts/PoolManagerLogic.sol†L516-L520】

* Права по настройкам протокола
  Не найдено в коде.

* Доступ к деньгам пользователей
  1. trader может перемещать активы пула через `execTransaction()` в рамках guard-ов.【F:contracts/PoolLogic.sol†L620-L654】
  2. trader не назначает роли и не управляет whitelist-ами.
  3. Минтинг вознаграждений не предусмотрен.

* Жёсткие ограничения
  trader действует только в рамках guard-ов и пока poolManager не отключил возможность менять активы trader адресу.【F:contracts/PoolLogic.sol†L620-L641】【F:contracts/PoolManagerLogic.sol†L516-L520】

#### investor

* Права по управлению активами пула
  `PoolLogic.deposit()` / `depositFor()` — investor вносит средства и получает долевые токены.【F:contracts/PoolLogic.sol†L222-L238】
  `PoolLogic.withdrawSafe()` / `withdrawToSafe()` — investor инициирует вывод своей доли активов из пула.【F:contracts/PoolLogic.sol†L387-L409】

* Права по настройкам протокола
  Не найдено в коде.

* Доступ к деньгам пользователей
  1. investor может вывести только собственную долю через функции withdraw.【F:contracts/PoolLogic.sol†L387-L517】
  2. investor не назначает другие адреса для вывода.
  3. Минтинг вознаграждений не предусмотрен.

* Жёсткие ограничения
  Депозит investor требует членства для приватных пулов, whitelist активов и минимального депозита; вывод возможен после кулдауна и с проверкой slippage/инвариантов.【F:contracts/PoolLogic.sol†L260-L347】【F:contracts/PoolLogic.sol†L412-L505】
...
* Права по управлению активами пула
  `PoolLimitOrderManager.executeLimitOrders()` / `executeSettlementOrders()` — authorizedKeeper исполняет лимитные и settlement ордера, перемещая долевые токены и вызывая EasySwapper для конвертации в базовые активы.【F:contracts/limitOrders/PoolLimitOrderManager.sol†L212-L381】

* Права по настройкам протокола
  Не найдено в коде.

* Доступ к деньгам пользователей
  1. authorizedKeeper управляет активами пользователей в рамках ордеров: переводит их долевые токены в EasySwapper и выкупает settlement токен, который отправляется пользователю.【F:contracts/limitOrders/PoolLimitOrderManager.sol†L320-L381】
  2. authorizedKeeper не может назначать новых адресов.
  3. Минтинг вознаграждений не предусмотрен.
...
* Права по управлению активами пула
  `PoolTokenSwapper.swap()` — swapWhitelist адреса могут обменивать активы и доли пулов по настройкам, которые задаёт poolTokenSwapperOwner.【F:contracts/swappers/poolTokenSwapper/PoolTokenSwapper.sol†L137-L225】

* Права по настройкам протокола
  Не найдено в коде.

* Доступ к деньгам пользователей
  1. swapWhitelist управляет только собственными активами; прямого доступа к чужим средствам нет.
  2. Не назначают новых участников.
  3. Минтинг наград не предусмотрен.

* Жёсткие ограничения
  Свопы возможны только по заранее включённым активам/пулам и при соблюдении minAmountOut; все операции идут через `whenNotPaused` и swapWhitelist, который контролирует poolTokenSwapperOwner.【F:contracts/swappers/poolTokenSwapper/PoolTokenSwapper.sol†L137-L225】

#### feeRecipient

feeRecipient — это daoAddress / адрес DAO или poolManager, который получает комиссии пула через `PoolLogic.mintManagerFee()`.
...
| ---- | ------------------------------ | ---------------------------- | ----------------------------- | ---------------------------------------- | ----------------------------------- |
| poolFactoryOwner | да | да | через ограничение: может менять активы пула и ставить пулы на паузу | нет | через ограничение: назначает feeRecipient и может инициировать mintManagerFee() |
| governanceOwner | да | нет | нет | нет | нет |
| assetHandlerOwner | да | нет | нет | нет | нет |
| easySwapperOwner | нет | нет | нет | нет | нет |
| limitOrderManagerOwner | нет | нет | нет | нет | нет |
| dhedgeEasySwapperOwner | через ограничение: setPoolAllowed | да | нет | да | да |
| poolTokenSwapperOwner | да | нет | нет | да | нет |
| poolManager | через ограничение: changeAssets() только whitelisted активы и лимиты фабрики | через ограничение: announceFeeIncrease() → commitFeeIncrease() с задержкой | через ограничение: execTransaction() под guard | через ограничение: execTransaction() под guard | да |
...
| investor | нет | нет | нет | через ограничение: выводит только свою долю после кулдауна | нет |
...
| authorizedKeeper | нет | нет | через ограничение: executeLimitOrders() в рамках ордера | через ограничение: исполняет ордер с переводом пользователю | нет |
| swapWhitelist | нет | нет | через ограничение: swap() только по заранее разрешённым активам/парам | нет | нет |
| feeRecipient | нет | нет | нет | нет | через ограничение: получает доли через mintManagerFee() |
